...
export class UserService {
   ...
   */
  static async getUsersByOfficeId(
    officeId: string
  ): Promise<{ data: UserResponse | null; error: any }> {
    try {
      const { data, error } = await supabase
        .from("users_with_office_assignment")
        .select("*")
   ...
   */
  static async createOfficeUser(
    officeUserData: CreateOfficeUserRequest
    ...
    try {
      ...
      if (
        !officeUserData.user_id ||
        !officeUserData.office_id ||
        !officeUserData.role_id
      ) {
      ...
      if (existingAssignment) {
        ...
        console.log(
          "⚠️ [CREATE_OFFICE_ASSIGNMENT] Duplicate assignment detected",
          {
      ...
      console.log(`datanya seperti berikut :${JSON.stringify(assignmentData)}`);
      ...
      console.log(
        "✅ [CREATE_OFFICE_ASSIGNMENT] Assignment created successfully",
        {
          status: "SUCCESS",
          data: {
  ...
  static async updateOfficeUser(
    ...
    try {
      ...
      console.log(
        "✅ [UPDATE_OFFICE_ASSIGNMENT] Assignment updated successfully",
        {
          status: "SUCCESS",
          data: {
            record_id: recordId,
  ...
  static async deleteOfficeUser(
    ...
    try {
      ...
      console.log(
        "✅ [DELETE_OFFICE_ASSIGNMENT] Assignment deleted successfully",
        {
          status: "SUCCESS",
          data: {
   ...
   */
  static async deleteAllUserOfficeAssignments(
    userId: string
  ): Promise<{ data: OfficeUser[] | null; error: any }> {
    ...
    try {
      ...
      console.log(
        "📊 [DELETE_ALL_USER_ASSIGNMENTS] Found existing assignments",
        {
      ...
      console.log(
        "✅ [DELETE_ALL_USER_ASSIGNMENTS] All assignments deleted successfully",
        {
          status: "SUCCESS",
          data: {
            user_id: userId,
   ...
   */
  static async getOfficeUserByUserId(
    userId: string
  ): Promise<{ data: OfficeUser | null; error: any }> {
    try {
      if (!userId) {
   ...
   */
  static async getAllOfficeAssignmentsByUserId(
    userId: string
  ): Promise<{ data: OfficeUser[] | null; error: any }> {
    try {
      if (!userId) {
   ...
   * Get all offices
   */
  static async getAllOffices(): Promise<{
    data: OfficeResponse | null;
    error: any;
  }> {
    try {
      const { data, error } = await supabase
   ...
   * Get office by ID
   */
  static async getOfficeById(
    officeId: string
  ): Promise<{ data: Office | null; error: any }> {
    try {
      if (!officeId) {
   ...
   * Get offices by type
   */
  static async getOfficesByType(
    type: string
  ): Promise<{ data: OfficeResponse | null; error: any }> {
    try {
      if (!type) {
   ...
   */
  static async getRolesByPermission(
    permission: string
  ): Promise<{ data: RoleResponse | null; error: any }> {
    try {
      if (!permission) {
  ...
  static async userHasPermission(
    ...
    try {
      if (!userId || !permission) {
        return {
          hasPermission: false,
          error: new Error("User ID and permission are required"),
        };
      }
      ...
      const hasPermission =
        data?.role_permissions?.includes(permission) || false;
